...
{
    class Space3FViewModel : Conductor<object>
    {
        ...
        private string alarmThirdTemp;
        public string AlarmThirdTemp
        {
            get => alarmThirdTemp;
            set
            {
                alarmThirdTemp = value;
                NotifyOfPropertyChange(() => AlarmThirdTemp);
            }
        }

        private string alarmFanStartTime;
        public string AlarmFanStartTime
        {
            get => alarmFanStartTime;
            set
            {
                alarmFanStartTime = value;
                NotifyOfPropertyChange(() => AlarmFanStartTime);
            }
        }

        private string alarmFanEndTime;
        public string AlarmFanEndTime
        {
            get => alarmFanEndTime;
            set
            {
                alarmFanEndTime = value;
                NotifyOfPropertyChange(() => AlarmFanEndTime);
            }
        }


        public Space3FViewModel()
        {
            InitDataFromDB();
            SetUp();
        }
        ...
        private void SetUp()
        {
            AlarmThirdTemp = Commons.AlarmThirdTemp;
            AlarmFanStartTime = Commons.AlarmFanStartTime;
            AlarmFanEndTime = Commons.AlarmFanEndTime;
        }
        ...
        public void Search()
        {
            ...
            using (var conn = new MySqlConnection(Commons.CONSTRING))
            {
                string strSelQuery = "Select ThirdFloorTemp,ThirdFloorHumid" +
                                     "  From tempandhumid " +
                                     " where " +
                ...
                try
                {
                    ...
                    while (reader.Read())
                    {
                        listTemps.Add(new DataPoint(i, Convert.ToDouble(reader["ThirdFloorTemp"])));
                        listHumids.Add(new DataPoint(i, Convert.ToDouble(reader["ThirdFloorHumid"])));
                        i++;
                    }
                    if (i == 0)
                    {
        ...
        public void Setup()
        {
            Commons.AlarmFanStartTime = AlarmFanStartTime;
            Commons.AlarmFanEndTime = AlarmFanEndTime;
            Commons.AlarmThirdTemp = AlarmThirdTemp;
            Commons.BROKERCLIENT.Publish("3F_tempconfig", Encoding.Default.GetBytes(AlarmThirdTemp));
            Commons.BROKERCLIENT.Publish("3F_AlramStartTimeconfig", Encoding.Default.GetBytes(AlarmFanStartTime));
            Commons.BROKERCLIENT.Publish("3F_AlramEndTimeconfig", Encoding.Default.GetBytes(AlarmFanEndTime));
        }
    }
}
